...
/* This version is only finding 2 nodes since it nos receiving Node** and using Node*.
	void searchHelper(Node* node, int depth, std::vector<float> target, float distanceTol, std::vector<int>& ids)
	{
		if (node != NULL)
		{
			...
			// Check relative position between target point and compared node.
			if ((target[depth%2] - distanceTol) < (node->point[depth%2])) // Point at left or below the node
				searchHelper(node->left, depth+1, target, distanceTol, ids);
			else if ((target[depth%2] + distanceTol) > (node->point[depth%2])) // Point at right or above the node
